...
(function (SaveLoad) {
    ...
    function gatherSaveData(name) {
        ...
        return { version: 1, name: name, timestamp: Date.now(), currentElevation: currentElevation, currentMap: curMap.name,
    ...
    function formatSaveDate(save) {
        ...
        return "".concat(date.getMonth() + 1, "/").concat(date.getDate(), "/").concat(date.getFullYear(), " ").concat(date.getHours(), ":").concat(date.getMinutes(), ":").concat(date.getSeconds());
    }
    ...
    function save(name, slot, callback) {
        ...
        console.log("[SaveLoad] Saving ".concat(1 + dirtyMapNames.length, " maps (current: ").concat(gMap.name, " plus dirty maps: ").concat(dirtyMapNames.join(", "), ")"));
        if (slot !== -1)
            save.id = slot;
        withTransaction(function (trans) {
