...
var Obj = (function () {
    ...
    Obj.fromPID_ = function (obj, pid, sid) {
        console.log("fromPID: pid=".concat(pid, ", sid=").concat(sid));
        ...
        if (pidType == 0) {
            ...
            console.log("invPID: ".concat(invPID, ", pid=").concat(pid));
            if (invPID !== 0xffff)
    ...
    Obj.prototype.loadScript = function (sid) {
        ...
        if (sid >= 0)
            ...
            if (this.pro.extra !== undefined && this.pro.extra.scriptID >= 0) {
                console.warn("PRO says sid is ".concat(this.pro.extra.scriptID & 0xffff, " (").concat(scriptName, "), but we're not ascribing it one (test)"));
            }
