...
(function (Ui) {
    ...
    var WindowFrame = (function () {
        function WindowFrame(background, bbox, children) {
            ...
            Object.assign(this.elem.style, {
                position: "absolute",
                left: "".concat(bbox.x, "px"),
                top: "".concat(bbox.y, "px"),
                width: "".concat(bbox.w, "px"),
                height: "".concat(bbox.h, "px"),
        ...
        Widget.prototype.hoverBG = function (background) {
            ...
            if (!this.elem.onmouseenter) {
                this.elem.onmouseenter = function () {
                    _this.elem.style.backgroundImage = "url('".concat(_this.hoverBackground, "')");
                };
                this.elem.onmouseleave = function () {
                    _this.elem.style.backgroundImage = "url('".concat(_this.background, "')");
                };
            }
            return this;
        };
        Widget.prototype.mouseDownBG = function (background) {
            ...
            if (!this.elem.onmousedown) {
                this.elem.onmousedown = function () {
                    _this.elem.style.backgroundImage = "url('".concat(_this.mouseDownBackground, "')");
                };
    ...
    var List = (function (_super) {
        ...
        List.prototype.select = function (item, itemElem) {
            if (!itemElem)
                itemElem = this.elem.querySelector("[data-uid=\"".concat(item.uid, "\"]"));
            if (!itemElem) {
                console.warn("Can't find item's element for item UID ".concat(item.uid));
                return false;
            }
    ...
    function initCharacterScreen() {
        ...
        Ui.characterWindow = new WindowFrame("art/intrface/edtredt.png", { x: Config.ui.screenWidth / 2 - 640 / 2, y: Config.ui.screenHeight / 2 - 480 / 2, w: 640, h: 480 })
            ...
            .add(new Label(33, 280, "Level: ".concat(critterGetStat(player, "Level"))).css({ fontSize: "0.75em", color: "#00FF00" }))
            .add(new Label(33, 292, "Exp: ".concat(critterGetStat(player, "Experience"))).css({ fontSize: "0.75em", color: "#00FF00" }))
            .add(new Label(380, 5, "Skill"))
            .add(new Label(399, 233, "Skill Points"))
            .add(new Label(194, 45, "Hit Points ".concat(critterGetStat(player, "HP"), "/").concat(critterGetStat(player, "Max HP")))
        ...
        var redrawStatsSkills = function () {
            ...
            for (var _i = 0, skills_1 = skills; _i < skills_1.length; _i++) {
                ...
                skillList.addItem({ text: "".concat(skill, " ").concat(newSkillSet.get(skill, newStatSet), "%"), id: skill });
            }
            for (var i = 0; i < stats.length; i++) {
                var stat = stats[i];
                statWidgets[i].setText("".concat(stat, " - ").concat(newStatSet.get(stat)));
            }
...
function uiContextMenu(obj, evt) {
    ...
    Object.assign($menu.style, {
        visibility: "visible",
        left: "".concat(evt.clientX, "px"),
        top: "".concat(evt.clientY, "px")
    });
...
function uiDrawWeapon() {
    ...
    $img("attackButtonType").src = "art/intrface/".concat(type, ".png");
...
function uiAddDialogueOption(msg, optionID) {
    $id("dialogueBoxTextArea").insertAdjacentHTML("beforeend", "<li><a href=\"javascript:dialogueReply(".concat(optionID, ")\">").concat(msg, "</a></li>"));
}
...
function uiLog(msg) {
    ...
    $log.insertAdjacentHTML("beforeend", "<li>".concat(msg, "</li>"));
...
function uiWorldMapShowArea(area) {
    ...
    $areamap.style.backgroundImage = "url('".concat(area.mapArt, ".png')");
    clearEl($areamap);
...
function uiElevator(elevator) {
    ...
    if (elevator.labels !== -1) {
        ...
        showv($elevatorLabel);
        $elevatorLabel.style.backgroundImage = "url('".concat(labelArt, ".png')");
    }
    var $elevatorBox = $id("elevatorBox");
    showv($elevatorBox);
    $elevatorBox.style.backgroundImage = "url('".concat(art, ".png')");
...
function uiCalledShot(art, target, callback) {
    ...
    drawChance("leftLeg");
    drawChance("rightLeg");
    $id("calledShotBackground").style.backgroundImage = "url('".concat(art, ".png')");
