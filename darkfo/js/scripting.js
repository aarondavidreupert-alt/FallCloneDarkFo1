...
(function (Scripting) {
    ...
    function warn(msg, type, script) {
        if (type !== undefined && Config.scripting.debugLogShowType[type] === false)
            return;
        if (script)
            console.log("WARNING [".concat(script._vm.intfile.name, "]: ").concat(msg));
        else
    ...
    function info(msg, type, script) {
        if (type !== undefined && Config.scripting.debugLogShowType[type] === false)
            return;
        if (script)
            console.log("INFO [".concat(script._vm.intfile.name, "]: ").concat(msg));
        else
    ...
    function dialogueExit() {
        ...
        if (currentDialogueObject) {
            ...
            vm.pc = vm.popAddr();
            info("[resuming from gsay_end (pc=0x".concat(vm.pc.toString(16), ")]"));
            vm.run();
        }
    ...
    var Script = (function () {
        ...
        Script.prototype.obj_can_see_obj = function (a, b) {
            ...
            if (!isGameObject(a) || !isGameObject(b)) {
                warn("obj_can_see_obj: not game object: a=".concat(a, " b=").concat(b), undefined, this);
                return 0;
            }
        ...
        Script.prototype.get_pc_stat = function (pcstat) {
            switch (pcstat) {
                ...
                case 5:
                    ...
                    return 0;
                default: throw "get_pc_stat: unhandled ".concat(pcstat);
            }
        };
        ...
        Script.prototype.override_map_start = function (x, y, elevation, rotation) {
            ...
            info("override_map_start: ".concat(x, ", ").concat(y, " / elevation ").concat(elevation));
    ...
    function timedEvent(script, userdata) {
        ...
        if (script.timed_event_p_proc === undefined) {
            warn("timedEvent called on script without a timed_event_p_proc! script: ".concat(script.scriptName, " userdata: ").concat(userdata));
            return false;
        }
