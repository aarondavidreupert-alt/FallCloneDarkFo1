...
function initGame() {
    ...
    if (location.search !== "") {
        ...
        if (query.indexOf("host=") === 0) {
            ...
            Netcode.connect("ws://".concat(host, ":8090"), function () {
                console.log("connected");
...
heart.load = function () {
    ...
    IDBCache.init(function () {
        cachedJSON("imageMap", "assets/art/imageMap.json", function (value) {
            imageInfo = value;
            cachedJSON("proMap", "assets/proto/pro.json", function (value) {
                proMap = value;
                initGame();
...
function isPassiveSkill(skill) {
    switch (skill) {
        ...
        default: throw "TODO: is passive skill ".concat(skill);
    }
}
