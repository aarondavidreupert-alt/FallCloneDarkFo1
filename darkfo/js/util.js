...
function getFileText(path, err) {
    ...
    xhr.send(null);
    if (xhr.status !== 200)
        throw Error("getFileText: got status ".concat(xhr.status, " when requesting '").concat(path, "'"));
...
function getFileBinarySync(path) {
    ...
    xhr.send(null);
    if (xhr.status !== 200)
        throw Error("getFileBinarySync: got status ".concat(xhr.status, " when requesting '").concat(path, "'"));
...
function assertEq(value, expected, message) {
    if (value !== expected)
        throw "AssertionError: value (".concat(value, ") does not match expected (").concat(expected, "): ").concat(message);
}
